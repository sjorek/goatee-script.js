...
(function() {
  ...
  exports.Interpreter = Interpreter = (function() {
    ...
    Interpreter.generate = _generate = (function() {
      var alias, aliases, clean, code, func, index, interpreter, key, pattern, runtime, unwrap, value, _i, _len, _ref2;
      aliases = [];
      ...
      runtime = {
        ...
        stack: {
          alias: index === 0 ? '__' : aliases[--index],
          code: 'null'
        },
        scope: {
          alias: index === 0 ? '$$' : aliases[--index],
          code: '[]'
        },
        evaluate: {
      ...
      pattern = [/(\s|\n)+/g, ' ', /_assignment/g, _operations['='].alias, /_evaluateRef/g, _operations['reference'].alias, /_context.call\(this, a\)/g, "{'$$':_global,'@':_variables}[a]", /(_g)lobal/g, runtime.global.alias, /(_v)ariables/g, runtime.variables.alias, /(_s)cope/g, runtime.scope.alias, /(_e)xecute/g, runtime.execute.alias, /(_b)ooleanize/g, runtime.booleanize.alias, /(__)slice\.call/g, runtime.arraySlice.alias, /(i)s(N)umber/g, runtime.global.alias, /(Nu)mber/g, runtime.Number.alias, /(i)s(F)unction/g, runtime.isFunction.alias, /(b)ind(F)unction/g, runtime.bindFunction.alias, /(_l)en/g, index === 0 ? "$1" : aliases[--index], /([a-z0-9])\s([^a-z0-9])/gi, '$1$2', /([^a-z0-9])\s([a-z0-9])/gi, '$1$2', /([^a-z0-9])\s([^a-z0-9])/gi, '$1$2', /([a-z0-9])\s([^a-z0-9])/gi, '$1$2', /([^a-z0-9])\s([a-z0-9])/gi, '$1$2', /([^a-z0-9])\s([^a-z0-9])/gi, '$1$2'];
      ...
      for (key in _operations) {
        ...
        interpreter.push([value.alias, '=', code, ';'].join(''));
      }
